...
A runbook for cloning the top-rated Kaggriculture agents using a free Kaggle
GPU. Everything needed is in this document; the script is also committed as
`kaggle_train_notebook.py`.

## Why this runs on Kaggle and not on a laptop

Two blockers, neither of them a modelling problem.

**The replays we want are behind a 23 GB table.** Episode ids are global across
every Kaggle simulation, so sampling them blindly pulls Card Battle or Halite
about fifteen times in sixteen, and there is no API that goes from a
leaderboard team to its episodes. Meta Kaggle publishes the mapping:

| file | size | what it gives |
|---|---|---|
| `EpisodeAgents.csv` | 23.1 GB | one row per agent per episode, with the rating it held |
| `Episodes.csv` | 6.8 GB | which competition each episode belongs to |

Together they yield the episode ids of any rating band. Thirty gigabytes is
hours to download; on a Kaggle Notebook the dataset **attaches with no download
at all**.

**And the training needs a GPU.** The machine this was developed on is ARM64,
12 CPU cores, no CUDA — JAX falls back to CPU and a pass that takes minutes on
a T4 takes hours. Kaggle Notebooks give a T4, often two, for 30 hours a week,
free, and the data is already sitting in the same filesystem.

## Why cloning is a reasonable bet here

The strong farms are **deterministic**. One opponent observed twice played
every crop identical to the tile — strawberry 47/47, wheat 10/10, melon 7/7 —
with melon planted day 1 and sold day 11 both times, and banked $24,421 in one
game and $112,872 in the other. The script is fixed; the variance is the draw.

A deterministic demonstrator is the easy case for imitation: one right answer
per state rather than a distribution. That is why a local DAgger run reached
parity with its expert. Pointing the same machinery at a ~3,150-rated
demonstrator raises the ceiling by roughly 2,400 rating points over fitting our
own ~714-rated heuristic.

## Setup

1. New Kaggle Notebook. **Accelerator: GPU T4 x2. Internet: ON.**
2. *Add data* → search **Meta Kaggle** → add `kaggle/meta-kaggle`.
3. *Add data* → upload this repo as a **private dataset** so `nn_features.py`
   and `train_target.py` are importable. Set `REPO` in the script to its path.
4. Paste the script into a cell and run.
5. Download `weights/leader.npz` from the notebook output.

Then run the agent against it locally:

```
KAG_LEARNED_UNITS=2 KAG_WEIGHTS=weights/leader.npz py -3.12 eval.py --games 12
```

`main.py` loads it through `KAG_WEIGHTS` exactly as it loads the DAgger
weights, so nothing else changes.
...
The dataset builder keeps only the **winning** seat of each episode, so it
imitates what a strong build does when it works. Roughly 240 replays at ~30 MB
each is a few gigabytes of notebook scratch and on the order of a million
labelled unit-targets.

Training reports held-out target accuracy against a "stay put" baseline. The
local equivalent reached 88 per cent against our own heuristic; a leader's
policy is a harder target, so expect lower — and judge it by game results, not
by accuracy. An earlier local policy hit 91 per cent per-action accuracy and
still banked $1, because per-action accuracy is not the objective.

## What this cannot do

- **It will not exceed the leaders.** Cloning reproduces a policy, not an
  understanding. The demonstrator is the ceiling.
- **A clone is only as robust as the states it was shown.** DAgger patched
  that locally by querying an expert on the states the policy actually
  reached; that is unavailable here, because there is no leader to ask about a
  state they never visited. Expect their state distribution to be the limit.
- **Verify before submitting.** Run `submit.py --dry-run`, which refuses on a
  crash, on a module-level import that will not exist on Kaggle, or on a
  per-turn p99 near the 1 s `actTimeout`.
